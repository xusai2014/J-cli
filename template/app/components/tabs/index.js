...
class TabItem extends Component {
  ...
  renderItem = (tab, index) => {
    ...
    /* if select item changed, fontSize can be change. ChengKai 20171213 */
    const textSizeStyle = (activeTab === index && activeTextSizes.length !== 0) ?
      { fontSize: activeTextSizes[0] } : { fontSize: activeTextSizes[1] };
    ...
    return (
      ...
      >
        <Text style={[styles.tabButtonText, { color: textColor }, textSizeStyle, tabItemStyle]}>
          {tab}
        </Text>
      </TouchableOpacity>
    );
